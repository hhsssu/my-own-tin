...
-- 가입일자도 추가해야하지 않을까? join_at, create_at, update_at, delete_at null -> date 프로시저 일정일자 지나면 진짜로 삭제.
...
-- -----------------------------------------------------
-- Table `myroutine`.`user_keyword` 프로필에 표시할 성격 키워드 (목표, mbti, 성격, 어디서 운동하는지 등등) (테이블명을 키워드로 바꿔야하나..?)
-- -----------------------------------------------------
-- DROP TABLE IF EXISTS `myroutine`.`user_keyword` ;

CREATE TABLE IF NOT EXISTS `myroutine`.`user_keyword` (
  `user_keyword_id` INT PRIMARY KEY AUTO_INCREMENT 	COMMENT '프로필키워드 고유번호',
  `keyword` VARCHAR(10) NOT NULL	COMMENT '프로필 키워드명', -- 한글 기준 5자 이내
  `users_user_id` INT NOT NULL		COMMENT '회원 고유번호', -- fk 연결
